...
def summarize(df: pd.DataFrame) -> pd.DataFrame:
    ...
    ssids = g["ssid"].agg(lambda s: ", ".join(sorted(set([x for x in s if x]))))
    chans = g["channel"].agg(lambda s: ", ".join(sorted(set([x for x in s if x]))))
...
def build_pdf(csv_path: Path, out_pdf: Path, meta: dict):
    ...
    doc = SimpleDocTemplate(
        ...
        leftMargin=18*mm, rightMargin=18*mm, topMargin=25*mm, bottomMargin=18*mm
    )
    ...
    hdr = Paragraph(f"<b>{title}</b>", styles["Title"])
    sub = Paragraph(f"{project}<br/>Generated: {when}", styles["Normal"])
    story += [hdr, Spacer(1, 6), sub, Spacer(1, 12)]
    ...
    story += [blurb, Spacer(1, 12)]

    table_header = ["#", "MAC (BSSID)", "SSID(s)", "Frames", "First Seen", "Last Seen",
                    "Min RSSI", "Avg RSSI", "Max RSSI", "Ch"]
    rows = [table_header]
    for i, row in enumerate(summarize(df).itertuples(index=False), start=1):
        rows.append([
            i,
            row.bssid or "(unknown)",
            (row.ssids or "")[:40],
            int(row.frames),
            row.first_seen.strftime("%Y-%m-%d %H:%M:%S") if pd.notna(row.first_seen) else "",
            row.last_seen.strftime("%Y-%m-%d %H:%M:%S") if pd.notna(row.last_seen) else "",
...
def main():
    ...
    meta = {"title": args.title, "project": args.project, "subtitle": args.subtitle}
